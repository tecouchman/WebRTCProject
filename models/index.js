...
module.exports.init = function(url, username, password, prefix, callback) {
    ...
    mongoose.connect('mongodb://' + username + ':' + password + '@' + url, function(err) {
		
		
		if (err) {
			console.log(err);
			
			// if an error occurred indicate it by 
			// setting connected to false;
			connected = false;
		} else {
