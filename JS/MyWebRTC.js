...
(function (global) {
    ...
    var close = function() {
        ...
        $.each(peers, function(key, obj) {
            obj.peerConnection.close();
        });
        $(MyWebRTC).trigger('close');
    }
    ...
    var addOffer =  function(remotePeerId, peerType, offer) {
        ...
        peers[remotePeerId].peerConnection.onaddstream = function(e) {
            ...
            $(MyWebRTC).trigger("GetRemoteStream", [ remotePeerId, peerType, e.stream ]);
        } 
    ...
    var addAnswer = function(remotePeerId, peerType, answer) {
    ...
    var sendMessage = function(message){
        ...
        $.map(peers, function(obj, key) {
            ...
            if (obj.peerConnection.sendDataChannel.readyState === 'open') {
                ...
                obj.peerConnection.sendDataChannel.send(JSON.stringify({ type : "message", data : sanitizedMessage, sender: localPeerId }));
            }
        });
    };
    ...
    var onMessageHandler = function(event) {
        ...
        if (receivedObj.type === 'message') {
            ...
            $(MyWebRTC).trigger("MessageReceived", [ receivedObj.data, receivedObj.sender ]);
        } else {
            // If filesharing is disable then ignore any recieved file data
            if (options.fileSharing) {
    ...
    var PeerConnectionFactory = {
        ...
        getPeerConnection : function(remotePeerId) {
            ...
            var peer = {};
            var peerCon = new MyWebRTC.PeerConnection(this.configuration, this.pcOptions);
    ...
    var MyWebRTC = {
        ...
        'close' : close,
        'peers' : peers,
        'setLocalPeerId': function(id) { localPeerId = id; },
